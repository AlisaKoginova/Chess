public class Rating {
    ...
    public static int rateAttack() {
        int counter=0;
        int tempPositionC=Game.kingPositionC;
        for (int i=0;i<64;i++) {
            switch (Game.chessBoard[i/8][i%8]) {
                case "P": {Game.kingPositionC=i; if (!Game.kingSafe()) {counter-=64;}}
                    break;
                case "R": {Game.kingPositionC=i; if (!Game.kingSafe()) {counter-=500;}}
                    break;
                case "K": {Game.kingPositionC=i; if (!Game.kingSafe()) {counter-=300;}}
                    break;
                case "B": {Game.kingPositionC=i; if (!Game.kingSafe()) {counter-=300;}}
                    break;
                case "Q": {Game.kingPositionC=i; if (!Game.kingSafe()) {counter-=900;}}
                    break;
            }
        }
        Game.kingPositionC=tempPositionC;
        if (!Game.kingSafe()) {counter-=200;}
        return counter/2;
    }
        ...
        for (int i=0;i<64;i++) {
            switch (Game.chessBoard[i/8][i%8]) {
                ...
                case "Q": counter+=queenBoard[i/8][i%8];
                    break;
                case "A": if (material>=1750) {counter+=kingMidBoard[i/8][i%8]; counter+=King.posible(Game.kingPositionC).length()*10;} else
                {counter+=kingEndBoard[i/8][i%8]; counter+=King.posible(Game.kingPositionC).length()*30;}
                    break;
            }
        }
        return counter;
